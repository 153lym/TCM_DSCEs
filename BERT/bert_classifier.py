...
def batch_convert_examples_to_features(
        examples, label_map, tokenizer, max_seq_length):
...
def main():
    ...
    # bert-base-chinese
    parser.add_argument("--bert_model", default='biobert_v1.1_pubmed/model.ckpt', type=str, required=True,
                        help="Bert pre-trained model selected in the list: bert-base-uncased, "
                             "bert-large-uncased, bert-base-cased, bert-large-cased, bert-base-multilingual-uncased, "
                             "bert-base-multilingual-cased, bbc.")
    ...
    parser.add_argument("--use_word_attention",
                        action='store_true',
                        help="Whether to use word attention layer.")
    ...
    model = BertForSequentialClassification.from_pretrained(args.bert_model,
                                                            ...
                                                            use_word_attention=args.use_word_attention,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(minibatches(train_examples, label_map, tokenizer,
                ...
                loss = model(input_ids, segment_ids, input_mask, document_mask, label_ids)
                if n_gpu > 1:
    ...
    if args.do_train:
        ...
        model = BertForSequentialClassification(config, num_labels=num_labels,
                                                ...
                                                rnn_hidden_size=args.rnn_hidden_size,
                                                dropout=args.dropout)
        model.load_state_dict(torch.load(output_model_file), False)
    elif args.do_resume:
        output_model_file = os.path.join(args.output_dir, WEIGHTS_NAME)
        output_config_file = os.path.join(args.output_dir, CONFIG_NAME)
        # Load a trained model and config that you have fine-tuned
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        eval_examples = processor.get_dev_examples(args.data_dir)  ## eval on dev/test sets
        ...
        for step, batch in enumerate(tqdm(minibatches(eval_examples, label_map, tokenizer,
            ...
            with torch.no_grad():
                preds = model(input_ids, segment_ids, input_mask, document_mask)
        ...
        with open(os.path.join(args.output_dir, 'eval_text'), 'w', encoding='utf-8') as ofile:
            for sent, pred, label in zip(eval_sents, preds_all, labels_all):
                # print(label_list[label], label_list[pred], sent)
                ofile.write('{}\t{}\t{}\n'.format(label_list[label], label_list[pred], sent))

        loss = tr_loss / nb_tr_steps if args.do_train else None
        result = {'eval_acc': eval_acc,
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        print(confusion_matrix(labels_all, preds_all))
        eval_sents = [sent for example in eval_examples for sent in example.document]
        with open(os.path.join(args.output_dir, 'test_results'), 'w', encoding='utf-8') as ofile:
            for sent, pred, label in zip(eval_sents, preds_all, labels_all):
                # print(label_list[label], label_list[pred], sent)
                ofile.write('{}\t{}\t{}\n'.format(label_list[label], label_list[pred], sent))
